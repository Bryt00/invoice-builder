        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-outline-variant/30 font-body text-sm text-on-surface">
            <tr v-for="user in users" :key="user.id" class="hover:bg-surface-container-low/40 transition-colors">
              ...
              <td class="px-6 py-4">
                <select :value="user.role?.name?.toLowerCase()" @change="e => onRoleSelectChange(user, (e.target as HTMLSelectElement).value)" class="bg-surface-container-high border border-outline-variant/40 rounded-lg px-2 py-1 font-label text-xs font-bold cursor-pointer focus:outline-none">
